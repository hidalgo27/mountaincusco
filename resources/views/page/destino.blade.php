...
<h3 class="md:text-5xl text-3xl font-semibold tracking-widest text-gray-50">Paquetes en {{$destino->nombre}}</h3>
...
<div class="lg:mx-20 md:mx-10 mx-5">
    ...
    <section class="md:px-5 px-2">
        @foreach ($paquetes as $precio_paquete)
        <div class="mb-16">
            <a href="/en-tours/{{$precio_paquete->paquetes->url}}" class="lg:hidden"><h3 class="text-2xl font-semibold text-primary pb-3 hover:text-secondary hover:font-light">{{$precio_paquete->paquetes->titulo}}</h3></a>
            <div class="grid lg:grid-cols-2 grid-cols-1 gap-8 items-center">
                <div class="relative">
                    <a href="/en-tours/{{$precio_paquete->paquetes->url}}"><img src="{{asset($precio_paquete->paquetes->imagen)}}" class="object-cover shadow-lg rounded transform hover:scale-105 transition duration-500"></a>
                    <div class=" absolute p-2 bottom-0 left-0 bg-primary bg-opacity-60 text-green-500 border-t border-r border-secondary">
                        <div class="cursor-pointer transform hover:scale-125 transition duration-500 font-black">{{$precio_paquete->paquetes->duracion}} días</div>
                    </div>
                </div>
                <div class="pr-5 py-2 border-r border-b border-secondary border-opacity-30 pb-5">
                    <a href="/en-tours/{{$precio_paquete->paquetes->url}}" class="hidden lg:block"><h3 class="text-2xl font-semibold text-primary pb-3 hover:text-secondary hover:font-light">{{$precio_paquete->paquetes->titulo}}</h3></a>
                    <div class="my-3 text-secondary italic font-semibold text-sm">
                        @foreach($precio_paquete->paquetes->paquetes_destinos as $paquete_destino)
                            {{$paquete_destino->destinos->nombre}}
                            @if ($loop->iteration < count($precio_paquete->paquetes->paquetes_destinos)) , @else . @endif
                        @endforeach
                    </div>
                    <div class="text-sm text-justify text-gray-600">{!!$precio_paquete->paquetes->descripcion!!}</div>
                    <div class="text-green-500 my-4 font-bold">
                        <span class="text-xs align-top">Desde</span>
                        {{$precio_paquete->paquetes->precio_paquetes->where('estrellas', 2)->first()->precio_d}}

                        <span class="text-xs">usd</span>
                    </div>
                    <div class="mt-3"><a href="{{route('paquete', $precio_paquete->paquetes->url)}}" class="text-sm bg-secondary bg-opacity-80 rounded p-2 text-white hover:text-primary hover:opacity-70">Ver detalles</a></div>
                </div>
            </div>
        </div>
        @endforeach
